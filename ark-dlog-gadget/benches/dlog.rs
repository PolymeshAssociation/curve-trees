...
use ark_dlog_gadget::dlog::{
  create_divisor_and_decomposition, commit_witness_chunks_prover, commit_witness_chunks_verifier,
  ...
  Sum<Params::ScalarBits, U1>: ArrayLength,
{
  ...
  for _ in 0..count {
    ...
    let (comm_orig, mut vars_orig) = prover.commit_vec(&[o_x, o_y], B::rand(&mut rng), &bp_gens);
    let o_y_var = vars_orig.pop().unwrap();
    let o_x_var = vars_orig.pop().unwrap();
    ...
    let proof = prover.prove(&bp_gens).unwrap();
    if proof_size == 0 {
      proof_size = proof.compressed_size() + comms.compressed_size() + comm_orig.compressed_size();
    }
