...
impl<C: DivisorCurve> ScalarMulAndDivisor<C> {
    ...
    where
        G: Clone,
    {
        let (divisor, point) = scalar.scalar_mul_divisor(generator_source)?;
        let divisor = divisor.normalize_x_coefficient()?;
        let aff = point.into_affine();
        let (x, y) = aff.xy().ok_or(Error::PointAtInfinity)?;
        Ok(ScalarMulAndDivisor { x, y, divisor })
    }
}
    ...
    fn test_on_curve() {
            ...
            C::Config: SWCurveConfig,
        {
            ...
            for _ in 0..count {
                ...
                let point = OnCurve {
                    x: LinearCombination::from(x_var),
                    y: LinearCombination::from(y_var),
                };
    ...
    fn test_incomplete_add_pub() {
        ...
        >() {
            ...
            for _ in 0..count {
                ...
                let point2_on_curve = OnCurve {
                    x: LinearCombination::from(x2_var),
                    y: LinearCombination::from(y2_var),
                };
                let sum_on_curve = OnCurve {
                    x: LinearCombination::from(sum_x_var),
                    y: LinearCombination::from(sum_y_var),
                };
