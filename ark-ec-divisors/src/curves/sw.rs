...
use crate::{DivisorCurve, Interpolator, XyPoint};
...
impl<C: SWCurveConfig> From<Projective<C>> for SwCurvePoint<C> {
    fn from(projective: Projective<C>) -> Self {
        Self(projective)
    }
}
...
impl<C: SWCurveConfig<BaseField: PrimeField> + Copy + PartialEq + Eq + HasInterpolator> DivisorCurve
    for SwCurvePoint<C>
{
    type Config = C;
