...
>(
    ...
    #[cfg(any(feature = "detailed_benchmarks", feature = "bench_prover"))]
    {
        ...
        group.bench_function("verification_gadget", |b| {
            b.iter(|| {
                // Common part
                let mut path_clone = paths[0].clone();
                curve_tree.add_root_to_randomized_path(&mut path_clone);

                let even_verification_gadget = || {
                    let even_transcript = MerlinTranscript::new(b"select_and_rerandomize");
                    let mut even_verifier = Verifier::new(even_transcript);
                    path_clone.even_verifier_gadget(&mut even_verifier, &sr_proof_params, &curve_tree);
                };

                let odd_verification_gadget = || {
                    let odd_transcript = MerlinTranscript::new(b"select_and_rerandomize");
                    let mut odd_verifier = Verifier::new(odd_transcript);
                    path_clone.odd_verifier_gadget(&mut odd_verifier, &sr_proof_params, &curve_tree);
                };
        ...
        group.bench_function("verification_tuples", |b| {
            b.iter(|| {
                // For naive approach, verify all paths together since proofs were generated for all paths
                let mut all_paths_with_root = Vec::new();
                for path in &paths {
                    let mut path_clone = path.clone();
                    curve_tree.add_root_to_randomized_path(&mut path_clone);
                    all_paths_with_root.push(path_clone);
                }

                let even_verification_gadget = || {
                    let even_transcript = MerlinTranscript::new(b"select_and_rerandomize");
                    let mut even_verifier = Verifier::new(even_transcript);
                    for path in &all_paths_with_root {
                        path.even_verifier_gadget(&mut even_verifier, &sr_proof_params, &curve_tree);
                    }
                    let _ = even_verifier
                ...
                let odd_verification_gadget = || {
                    ...
                    let mut odd_verifier = Verifier::new(odd_transcript);
                    for path in &all_paths_with_root {
                        path.odd_verifier_gadget(&mut odd_verifier, &sr_proof_params, &curve_tree);
                    }
                    let _ = odd_verifier
            ...
            |b, proofs| {
                b.iter(|| {
                    #[cfg(feature = "parallel")]
                    {
                        ...
                        rayon::join(
                            || {
                                let even_verification_scalars_and_points: Vec<_> = proofs.par_iter()
                                    .map(|paths| {
                                        let even_transcript =
                                        ...
                                        let mut odd_verifier = Verifier::new(odd_transcript);
                                        for i in 0..M {
                                            let _ = paths[i].select_and_rerandomize_verifier_gadget(
                                                &root,
                                                &mut even_verifier,
                                                &mut odd_verifier,
                                                &sr_proof_params,
                            ...
                            || {
                                let odd_verification_scalars_and_points: Vec<_> = proofs.par_iter()
                                    .map(|paths| {
                                        let even_transcript =
                                            MerlinTranscript::new(b"select_and_rerandomize");
                                        let mut even_verifier = Verifier::new(even_transcript);
...
>(
    ...
    let commitments_to_select: Vec<_> = (0..M).map(|_| Affine::<P0>::rand(&mut rng)).collect();
    let indices = (0..M as u32).collect::<Vec<u32>>();
    let curve_tree =
        CurveTree::<L, M, P0, P1>::from_leaves(&commitments_to_select, &sr_params, Some(depth));
    ...
    #[cfg(any(feature = "detailed_benchmarks", feature = "bench_prover"))]
    {
        ...
        group.bench_function("verification_gadget", |b| {
            b.iter(|| {
                // Common part
                let mut path_clone = multi_path.clone();
                curve_tree.batched_select_and_rerandomize_verification_commitments(&mut path_clone);

                let even_verification_gadget = || {
                    let even_transcript = MerlinTranscript::new(b"select_and_rerandomize");
                    let mut even_verifier = Verifier::new(even_transcript);
                    path_clone.even_verifier_gadget(&mut even_verifier, &sr_proof_params, &curve_tree).unwrap();
                };

                let odd_verification_gadget = || {
                    let odd_transcript = MerlinTranscript::new(b"select_and_rerandomize");
                    let mut odd_verifier = Verifier::new(odd_transcript);
                    path_clone.odd_verifier_gadget(&mut odd_verifier, &sr_proof_params, &curve_tree).unwrap();
                };
                            ...
                            || {
                                let even_verification_scalars_and_points: Vec<_> = srvs
                                    .map(|srv| {
                                        ...
                                        srv.batched_select_and_rerandomize_verifier_gadget(
                                            &root,
                                            &mut even_verifier,
                                            &mut odd_verifier,
                                            &sr_proof_params,
                                        )
                                        .expect("Failed to run verifier gadget");
