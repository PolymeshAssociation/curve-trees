...
fn insert_only() {
    ...
    check_updates::<2, PallasBase, PallasConfig, VestaConfig>(3, 11, (0..8).into_iter().collect(), 0, None);
    check_updates::<2, PallasBase, PallasConfig, VestaConfig>(4, 11, (0..16).into_iter().collect(), 0, None);
    check_updates::<2, PallasBase, PallasConfig, VestaConfig>(5, 12, (0..32).into_iter().collect(), 0, None);
    check_updates::<4, PallasBase, PallasConfig, VestaConfig>(1, 11, (0..4).into_iter().collect(), 0, None);
    check_updates::<4, PallasBase, PallasConfig, VestaConfig>(2, 11, (0..16).into_iter().collect(), 0, None);
    check_updates::<4, PallasBase, PallasConfig, VestaConfig>(3, 11, (0..64).into_iter().collect(), 0, None);
}
...
fn insert_and_update_small() {
    ...
    check_updates::<2, PallasBase, PallasConfig, VestaConfig>(4, 11, vec![1, 3, 5, 7, 9, 11], 2, None);
    check_updates::<2, PallasBase, PallasConfig, VestaConfig>(4, 11, vec![2, 4, 6, 8, 10, 12], 2, None);
...
fn insert_and_update_large() {
    check_updates::<256, PallasBase, PallasConfig, VestaConfig>(2, 13, vec![256, 512, 513, 2048], 100, None);
    check_updates::<256, PallasBase, PallasConfig, VestaConfig>(3, 13, vec![256, 512, 513, 2048], 100, None);
}
...
>(
    ...
    let num_leaves = (leaf_indices_to_track.last().unwrap() + 1 + num_updates_after_last_tracked_leaf) as usize;
    ...
    for i in 0..num_leaves {
        ...
        if leaf_indices_to_track.len() > leaves_inserted && leaf_indices_to_track[leaves_inserted] == i as u64 {
            if leaves_inserted != 0 {
                ...
                partial_curve_tree.update_on_leaves(leaves[(leaf_index_to_prove+1) as usize..i].to_vec(), &sr_params).unwrap();
                
                // Get path for previous leaf from partial tree
                let path = partial_curve_tree.get_path_to_leaf(leaf_index_to_prove).unwrap();
                
                // Create and verify proof for the previous leaf
                let root = partial_curve_tree.root_node();
                check_proof(&mut rng, leaves[leaf_index_to_prove as usize], path, &root, &sr_params)
                
            }
            partial_curve_tree.insert_leaf(i as u64, leaves[i], lean_curve_tree.odd_level_path_nodes.clone(), lean_curve_tree.even_level_path_nodes.clone()).unwrap();
            leaves_inserted += 1;
        }
    }
    ...
    partial_curve_tree.update_on_leaves(leaves[(leaf_index_to_prove+1) as usize..].to_vec(), &sr_params).unwrap();
    let path = partial_curve_tree.get_path_to_leaf(leaf_index_to_prove).unwrap();
    let root = partial_curve_tree.root_node();
    check_proof(&mut rng, leaves[leaf_index_to_prove as usize], path, &root, &sr_params);
