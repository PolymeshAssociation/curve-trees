    ...
    > CurveTree<L, M, P0, P1>
{
    ...
    pub fn select_and_rerandomize_prover_multi_witness(
        ...
        for (tree_index, leaf_index) in indices.iter().enumerate() {
            independent_paths.push(self.get_path_to_leaf_for_proof(*leaf_index, tree_index));
        }
                ...
                if i < odd_length - 1 {
                    ...
                    for (inclusion_index, chunk) in chunks.enumerate() {
                        single_level_select_and_rerandomize(
                            ...
                            chunk.to_vec(),
                            Some(
                                (self.odd_internal_nodes[i][inclusion_index]
    ...
    pub fn single_level_batched_select_and_rerandomize_prover_gadget<const M: usize>(
        ...
        let (selected_children_plus_delta, sum_of_selected) = {
            ...
            for i in 0..M {
                ...
                children[i] =
                    (nodes[i].child_node_to_randomize + odd_parameters.delta).into_affine();
            }
