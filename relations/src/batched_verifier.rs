    ...
    >(
            ...
            Root::Even(root_node) => {
                ...
                let shifted_rerandomized = (self.path.odd_commitments[0]
                    + (parameters.odd_parameters.sl_params.delta
                        * P1::ScalarField::from(num_indices)))
                .into_affine();
            ...
            Root::Odd(root_node) => {
                ...
                let shifted_rerandomized = (self.path.even_commitments[0]
                    + (parameters.even_parameters.sl_params.delta
                        * P0::ScalarField::from(num_indices)))
                .into_affine();
                let (x, y) = shifted_rerandomized.xy().unwrap();
        ...
        let mut commit_even = || {
            ...
            for parent_index in 0..even_length {
                ...
                let p = commit_dlog_and_divisor::<_, _, Parameters0>(
                    even_verifier,
                    &self.even_divisor_comms[child_index],
                );
                even_node_divisors.push((sum_x_var, sum_y_var, x, y, p));
            }
        };
        ...
        let mut commit_odd = || {
            ...
            for parent_index in 0..odd_length {
                ...
                if parent_index < (odd_length - 1) {
                    let children_vars: Vec<Variable<F1>> = odd_verifier.commit_vec(
                        L * num_indices as usize,
                        self.path.odd_commitments[parent_index],
                    );
                    ...
                    let p = commit_dlog_and_divisor::<_, _, Parameters1>(
                        odd_verifier,
                        &self.odd_divisor_comms[child_index],
                    );
                    odd_node_divisors.push((sum_x_var, sum_y_var, x, y, p));
                }
            }
        ...
        // Process leaf level - individual verification for each leaf
        {
            ...
            for (i, chunk) in chunks.iter().enumerate() {
                ...
                let rerandomized_plus_delta = (self.path.selected_commitments[i]
                    + parameters.even_parameters.sl_params.delta)
                    .into_affine();
                ...
                let p = commit_dlog_and_divisor::<_, _, Parameters1>(
                    ...
                    &self.odd_divisor_comms
                        [self.odd_divisor_comms.len() - num_indices as usize + i],
                );
