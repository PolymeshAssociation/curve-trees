    ...
    > CurveTreeNode<L, M, P0, P1>
{
    ...
    pub fn generate_witness_node_for_this_and_children(
        ...
        if let Self::InnerNode(inner_node) = &self {
            ...
            let child_node_to_rerandomize =
                inner_node.get_child(child_node_index_to_rerandomize)?;
    ...
    > CurveTree<L, M, P0, P1>
{
        ...
        match self {
            Self::Even(ct) => {
                if let CurveTreeNode::InnerNode(inner_node) = ct {
                    let child_nodes_to_randomize = Self::generate_paths(
                        leaf_indices,
                        tree_index,
                        &mut even_internal_nodes,
                        &mut odd_internal_nodes,
            ...
            Self::Odd(ct) => {
                if let CurveTreeNode::InnerNode(inner_node) = ct {
                    let child_nodes_to_randomize = CurveTree::<L, M, P1, P0>::generate_paths(
                        leaf_indices,
                        tree_index,
                        &mut odd_internal_nodes,
                        &mut even_internal_nodes,
    ...
    fn generate_paths(
        ...
        let num_leaves = leaf_indices.len();
        // Get children of root nodes for each leaf
        ...
        for leaf_index in leaf_indices {
            ...
            // child_node_to_randomize might be same for many leaves but still added since the
        ...
        // Process children of each child
...
{
    /// List of witnesses of generated by taking children of even level internal nodes. The children
    /// are themselves odd level nodes.
    pub even_internal_nodes: Vec<WitnessNode<L, P0, P1>>,
    /// List of witnesses of generated by taking children of odd level internal nodes. The children
    /// are themselves even level nodes.
...
#[derive(Clone, CanonicalSerialize, CanonicalDeserialize, Zeroize, ZeroizeOnDrop)]
pub struct WitnessPathsWithSameRoot<
    const L: usize,
    P0: SWCurveConfig + Copy,
...
impl<const L: usize, P0: SWCurveConfig + Copy, P1: SWCurveConfig + Copy>
    ...
    > CurveTreeWitnessPath<L, P0, P1>
{
    ...
    pub fn select_and_rerandomize_prover_gadget<R: CryptoRngCore>(
        ...
        // The leaf's rerandomization scalar is the last even rerandomization scalar; capture it
        // before `even_rerandomization_scalars` is moved into the non-root gadget below.
    ...
    pub fn select_and_rerandomize_prover_gadget_for_common_root<R: CryptoRngCore>(
        ...
        (
            Vec<SelectAndRerandomizePath<L, P0, P1>>,
            Vec<P0::ScalarField>,
        ),
        Error,
    > {
        ...
        let selected_children_x_coords = if is_root_even {
            ...
            let witness_nodes_of_root = paths
                .iter()
                .map(|p| &p.even_internal_nodes[0])
                .collect::<Vec<_>>();
            let delta = parameters.odd_parameters.sl_params.delta;
            let x_coords_selected_children =
                Self::process_root_nodes_for_given_paths_with_common_root(
    ...
    > {
        ...
        for path in paths {
            ...
            // The leaf's rerandomization scalar is the last even rerandomization scalar; capture
            // it before `even_rerandomization_scalars` is moved into the non-root gadget below.
            let re_randomization_of_leaf = even_rerandomization_scalars
                .last()
                .copied()
                .unwrap_or_default();
    ...
    /// Randomizes the nodes (by adding `B_blinding * r_i`) in the witness path and returns the randomized nodes and randomizers.
    /// Returns: (even_rerandomized_nodes, odd_rerandomized_nodes, even_rerandomization_scalars, odd_rerandomization_scalars)
    /// odd_rerandomized_nodes are the nodes at odd level but randomized with scalars from odd_rerandomization_scalars, same for even_rerandomized_nodes
    pub(crate) fn randomize_nodes<R: CryptoRngCore>(
        ...
        // For each node on even levels in the witness path, randomize its child (odd level node) by adding `B_blinding * r_i`
        ...
        // by adding `B_blinding * r_i`. Since the leaf is always at even level, the parent of leaf is always at
        // odd level, so the last (lowest) odd node's `child_node_to_randomize` is the leaf being proven. Its
        // rerandomization scalar is therefore the last element of `even_rerandomization_scalars`.
        for odd in &self.odd_internal_nodes {
...
pub(crate) fn allocate_children_of_root_and_enforce_membership<
    P: SWCurveConfig,
    Cs: RandomizableConstraintSystem<P::BaseField>,
>(
    cs: &mut Cs,
    ...
    let x_coords_selected_children = match child_nodes_to_randomize {
        Some(child_nodes_to_randomize) => child_nodes_to_randomize
            .into_iter()
            .map(|c| cs.allocate(Some(c.x)).unwrap().into())
            .collect::<Vec<_>>(),
        _ => (0..num_selected_children)
            .map(|_| cs.allocate(None).unwrap().into())
            .collect::<Vec<_>>(),
    };
    ...
    >(
        ...
        match self {
            Self::Even(coords) => {
                ...
                let child_plus_delta = path.map(|p| {
                    (p.even_internal_nodes[0].child_node_to_randomize
            ...
            Self::Odd(coords) => {
                ...
                let x_var = coords.remove(0);
                let child_plus_delta = path.map(|p| {
                    (p.odd_internal_nodes[0].child_node_to_randomize
    ...
    for RootChildren<L, P0, P1>
{
    fn deserialize_with_mode<R: Read>(
        ...
        match variant {
            ...
            1 => {
                let x_coords =
                    <[P1::ScalarField; L]>::deserialize_with_mode(&mut reader, compress, validate)?;
                let child_nodes_to_randomize =
    ...
    fn check(&self) -> Result<(), SerializationError> {
        match &self {
            Self::Even {
                ...
                x_coords.check()?;
                child_nodes_to_randomize.check()?;
            }
            Self::Odd {
                x_coords,
                child_nodes_to_randomize,
